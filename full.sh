...
ARCHIVE_PREFIX="$PROJECT-backup-full"
...
function time_to_backup() {
    ...
    next_ts=$(date -d "$last_date +7 days" +%s)
...
if server_is_up; then
    if time_to_backup || is_first_day_of_month; then
        ...
        echo "Резервная копия была менее 7 дней назад. Пропускаем."
    fi
else
